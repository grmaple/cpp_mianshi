...
166. 定义递增和递减运算符的类，应该同时定义前置版本和后置版本。这些运算符通常会被定义成类的成员。前置运算符应该返回递增或递减后对象的引用。A &operator++();为了区分前置和后置版本，后置版本接受一个额外的int类型形参内置为0。A operator(int);后置运算符应该返回对象的原值，返回形式是一个值而非引用。
