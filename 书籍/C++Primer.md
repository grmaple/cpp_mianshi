...
162. 重载输出运算符：ostream &operator<<(ostream &os, const A &a){return os;}第一参数是非常量ostream对象的引用(向流写内容会改变其状态，无法直接复制一个ostream对象)，第二参数是常量的引用(避免复制实参，一般不改变对象的内容)，返回值是ostream形参的引用(把原对象返回，可以连续使用<<)。输出运算符尽量减少格式化操作(换行符)，应该让用户有权控制输出的细节。
163. 输入输出运算符必须是非成员函数，并且声明为友元。
164. 重载输入运算符>>：istream &operator>>(istream &is, A &a){return is;}输入运算符还需要处理可能失败的情况，if(is){}else{}
165. 如果一个类包含下标运算符，则它通常定义两个版本：一个是返回普通引用，一个类的常量成员函数并且返回常量引用。
166. 定义递增和递减运算符的类，应该同时定义前置版本++i和后置版本i++。这些运算符通常会被定义成类的成员。前置运算符应该返回递增或递减后对象的引用。A &operator++();为了区分前置和后置版本，后置版本接受一个额外的int类型形参内置为0。A operator(int);后置运算符应该返回对象的原值，返回形式是一个值而非引用。
167. 箭头运算符不执行任何自己的操作，而是调用解引用运算符并返回解引用结果元素的地址。解引用运算符返回curr所指元素的一个引用。
168. 如果类重载了函数调用运算符，那么我们可以像使用函数一样使用该类的对象。这被称为函数对象，仿函数，其同时可以存储状态，所以比一般函数更加灵活。当我们编写了一个lambda后，编译器将该表达式翻译成一个未命名类的未命名对象。在lambda表达式产生的类中含有一个重载的函数调用符。
169. 标准库function类型。
        1. function<int(int,int)> f1= add;函数指针
        2. function<int(int,int)> f2= divide();函数对象
        3. function<int(int,int)> f3 =[](int i,int j){return i*j;};lambda
170. 类型转换运算符，是类的一种特殊成员函数。用于将对象转换成内置类型。不能声明返回类型，形参类型也必须为空，通常是const函数。operator int()const{return val;}
171. 显式的类型转换运算符explicit，如此编译器将不会进行隐式转换。static_cast<int>(s)+3;可以通过显式的强制类型转换。该规定存在一个例外，即表达式被用作条件时，显式的类型转换将被隐式地执行：
        1. if，while及do语句的条件部分
        2. for语句头的条件表达式
        3. !,&&,||的运算对象
        4. ?:的条件表达式
172. 避免有二义性的类型转换。通常情况下不要为类定义相同的类型转换，也不要在类中定义两个及两个以上转换源或转换目标是算术类型的转换。
173. 基类通常都应该定义一个虚析构函数，即使该析构函数不执行任何实际操作也是如此。基类必须将它的两种成员函数区分开来：一种是基类希望派生类进行覆盖的函数，一种是基类希望派生类直接继承而不要改变的函数。前者通常定义为虚函数，当使用指针或引用调用虚函数时，该调用将被动态绑定，运行版本由实参决定，即在运行时选择函数的版本。任何构造函数之外的非静态函数都可以是虚函数。非虚成员函数其解析过程发生在编译时而非运行时。
174. 派生类必须将其继承而来的成员函数中需要覆盖的那些函数重新声明。
175. 因为在派生类对象中含有与其基类对应的组成部分，所以我们能把派生类的对象当成基类对象来使用。而且我们也能将基类的指针或引用绑定到派生类对象中的基类部分上。这种转换通常被称为派生类到基类的类型转换，也就是向上转换，这是隐式的。还能在类名后加final表示该类不能被继承。
176. 当我们使用存在继承关系的类型时，必须将一个变量或其他表达式的静态类型与该表达式表示对象的动态类型区分开。表达式的静态类型在编译时总是已知的，它是变量声明时的类型或表达式生成的类型。动态类型则是变量或表达式表示的内存中的对象的类型。动态类型，直到运行时才可知。如果表达式既不是引用也不是指针，则它的动态类型永远是和静态类型一致。
177. 动态绑定，只有当我们通过指针或引用调用虚函数时才会发生。当我们通过一个具有普通类型的表达式调用虚函数时，在编译时就会将调用的版本确定下来。
178. OOP的核心是多态性。我们把具有继承关系的类型称为多态类型。因为我们能使用这些类型的多种形态，无需在意他们的差异。指针或引用的静态类型与动态类型不同这一事实正是c+）语言支持动态性的根本所在。
179. override用于派生类虚函数，允许派生类显式地注明它使用某个成员函数覆盖了它继承的虚函数,如此，如果写错了参数，没有覆盖已存在的虚函数，那么编译器将报错了。final用于基类虚函数，表示任何覆盖该函数的操作都引发错误。
180. 在某些情况下，我们希望对虚函数的调用不要进行动态绑定，而是强迫其执行虚函数的某个特定版本，比如被覆盖的基类版本。可以使用作用域运算符实现这一目的。
181. 纯虚函数，清晰明了地告诉用户这个函数是没有意义的，必须重新定义。含有纯虚函数的类是抽象类。抽象基类负责定义接口，而后续的其他类可以覆盖该接口。我们不能直接创建一个抽象基类的对象。
182. 继承构造函数，派生类直接重用直接基类的构造函数。class A : public B {public: using B::B;}和普通成员的using声明不一样。一个构造函数的using声明不会改变该构造函数的访问级别。而且一个using声明一句不能指定explicit或constexpr。
183. 在容器中放置(智能)指针而非对象。一般放基类的指针。该基类指针就可以指向基类或者是派生类了。
184. 定义一个函数模板，而不是为每个类型都定义一个函数。template<typename T>int compare(const T&v1,const T&v2);一个函数模板就是一个公式，可用来生成针对特定类型的函数版本。模板参数参数表示在类或函数定义中用到的类型或指。当使用模板时，我们指定模板实参，将其绑定到模板参数上。
185. 编译器用推断出的模板参数来为我们实例化一个特定版本的函数。compare(1,0);这些编译器生成的版本通常被称为模板的实例。
186. 模板类型参数class T或者typename U。除了定义类型参数还能定义非类型参数。一个非类型参数表示一个值而非一个类型。当一个模板被实例化时，非类型参数被一个用户提供的或编译器推断出的值所代替。这些值必须是常量表达式，从而允许编译器在编译时实例化模板。非类型模板参数的模板实参必须是常量表达式。在需要常量表达式的地方，可以使用非类型参数，比如指定数组大小。
187. 当编译器遇到一个模板定义时，它并不生成代码。只有当我们实例化出模板的一个特定版本时，编译器才会生成代码。因此模板的头文件通常包括声明和定义。
188. 类模板是用来生成类的蓝图的。与函数模板不同的是，编译器不能为类模板推断模板参数类型。为了使用类模板，必须在模板名后的尖括号中提供额外信息--用来代替模板参数的模板实参列表。实例化类模板，这些额外信息是显式模板实参列表，它们被绑定到模板参数，编译器使用这些模板实参来实例化出特定的类。
189. 默认情况下，一个类模板的成员函数只有当程序用到它时才进行实例化。在一个类模板的作用域内，我们可以直接使用模板名而不必指定模板实参。
190. 可以为类模板定义一个类型别名。template<typename T>using twin = pair<T,T>;twin<int> x;//相当于pair<int,int>;还可以固定一个或多个模板参数。template<typename T>using twin = pair<T,int>;twin<string> x;//相当于pair<string,int>;
191. 为了处理模板，编译器必须知道名字是不是表示一个类型（类型，还是static数据成员）。默认情况下作用域访问的名字不是类型，如果希望使用一个模板类型参数的类型，需要显式使用typename说明该名字是一个类型。
192. 一个类可以包含本身是模板的成员函数，被称为成员模板，成员模板不能是虚函数。
193. 当模板被使用时才会进行实例化的特性意味着相同的实例可能出现在多个对象文件中，如此开销可能会非常严重。显式实例化可以避免这种开销。对每个实例化声明，在程序的某个位置必须有其显式的实例化定义。在一个类模板的实例化定义中，所用类型必须能用于模板的所有成员函数。
194. 如果函数参数类型不是模板参数，则对实参进行正常的类型转换。
195. 后置返回类型允许我们在参数列表后声明返回类型。template<typename T>auto fun(T beg, T end)->decltype(*beg){return *beg;}
196. 为了获得元素类型，我们可以使用标准库的类型转换模板。头文件type_traits。这个头文件的类通常用于所谓的模板元程序设计，在普通编程中也很有用。使用remove_reference来获得元素类型。remove_reference模板有一个模板类型参数和一个名为type的公有类型成员。remove_reference<int&>，type将是int。template<typename T>auto fun(T beg, T end)->typename remove_reference<decltype(*beg)>::type{return *beg;}
197. 我们如果间接创建一个引用的引用，则这些引用形成了“折叠”。一般情况下，引用会折叠成一个普通的左值引用类型。只有在右值引用的右值引用时，引用才会折叠成右值引用。
        1. X& &,X& &&和X&& &都折叠成X&
        2. X&& &&折叠成X&&

引用折叠只能应用于间接创建的引用的引用，如类型别名或模板参数。

如此，可以将任意类型的实参传递给T&&类型的函数参数。

198. 虽然不能直接将一个右值引用绑定到一个左值上，但可以使用move获得一个绑定到左值的右值引用。move可以接受任何类型的实参，这是一个函数模板。
199. 某些函数需要将其一个或多个实参连同类型不变地转发给其他函数，在此情况下需要保持被转发实参的所有性质，包括实参类型是否是const的以及实参是左值还是右值。如果一个函数参数是指向模板类型参数的右值引用(T&&)，它对应的实参的const属性和左值/右值属性将得到保持。接受一个左值引用，但不能接受右值引用。使用std::forward保持类型信息。forward必须通过显式模板实参来调用。当用于一个指向模板类型参数的右值引用函数参数(T&&)时，forward会保持实参类型的所有细节。
200. 可变参数模板就是一个接受可变数目参数的模板函数或模板类。可变数目的参数被称为参数包。存在两种参数包：模板参数包，表示零个或多个模板参数；函数参数包，表示零个或多个函数参数。sizeof...返回一个常量表达式，可知道一个包有多少元素。
201. 可变参数函数一般是递归的。第一步调用处理包中的第一个实参，然后用剩余实参调用自己。为了终止递归，我们还需要定义一个接受非可变参数的同名函数。
202. 对于一个参数包，我们唯一能做的事情就是扩展它。当扩展一个包时，我们还要提供用于每个扩展元素的模式。扩展一个包就是将它分解为构成的元素，对每个元素应用模式，获得扩展后的列表。我们通过在模式右边放...来触发扩展。扩展中的模式会独立地应用于包中的每个元素，模式可以是函数。
203. 转发参数包，可以使用可变参数模板和forward机制编写函数，实现将实参一个不变地传递给其他函数。
204. 模板特例化，当我们不能或者不希望使用模板版本时，可以定义类或函数模板的一个特例化版本。
205. 特例化本质上是实例化一个模板，而非重载它。因此特例化不影响函数匹配。
206. 与函数模板不同，类模板的特例化不必为所有模板参数提供实参。我们可以只指定一部分而非所有模板参数，或是参数的一部分而非全部特性。我们只能部分特例化类模板，而不能部分特例化函数模板。可以只特例化特定成员函数而不是特例化整个模板。
207. tuple类型，可以存放不同类型，任意数量的成员，类似pair。每个确定的tuple类型的成员是数目固定的。这是个“快速而随意”的数据结构。
208. tuple初始化，需要指定每个成员的类型
        1. tuple<size_t,size_t,size_t>threeD;默认构造函数，值初始化

tuple<size_t,size_t,size_t>threeD{1,2,3};直接初始化

    2. tuple<size_t,size_t,size_t>threeE(1,2,3);构造函数，初始化
    3. auto threeF=make_tuple(1,2,3);make_tuple函数生成tuple对象
209. 访问tuple成员，使用get()标准库函数模板。auto a=get<0>(threeF);返回threeF的第一个成员。使用tuple从一个函数返回多个值。
210. bitset类型，使得位运算的使用更容易，并且能够处理超过最长整型类型大小的位集合，类似array。编号从0开始的二进制位被称为低位，编号到31结束的二进制位被称为高位。bitset<32>bitvec(1U);bitvec位数比初始值多，高位置0，如果bitvec位数比初始值少，初始值高位被丢弃。可以使用一个string或一个字符数组指针来初始化bitset，两种情况下字符都直接表示位模式。bitset<8>bitvc("1100");表示00001100
211. 正则表达式RE库。regex类表示一个正则表达式。函数regex_match和regex_search确定一个给定字符序列与一个给定regex是否匹配。如果整个输入序列和表达式匹配，则regex_match函数返回true；如果输入序列中一个子串与表达式匹配，则regex_search函数返回true。只返回第一个结果。
212. 默认regex使用的正则表达式语言是ECMAScript。string pattern="[[:alpha:]]*"+"[^c]ei"+"[[:alpha:]]*";[^c]匹配不是'c'的字符。[[:alpha:]]匹配任意字母。+表示匹配一个或多个，*表示匹配零个或多个。regex r(pattern);regex对象，smatch result;存储搜索结果。if(regex_search(test_str,result,r))cout<<result.str();
213. 指定regex对象的选项。icase忽略大小写。regex r("[[:alnum:]]+\\.(cpp|cxx|cc)$",regex::icase);[[:alnum:]]匹配任意字母或数字。.是特殊字符，通常匹配任意字符，\\去掉其特殊含义。
214. 如果编写的正则表达式有错误，在运行时会抛出regex_error异常。正则表达式的编译是非常缓慢的操作，其在运行时才编译。所以应该避免创建不必要的正则表达式。
215. 使用sregex_iterator迭代器来获取所有匹配。for(sregex_iterator it(file.begin(),file.end(),r),end_it;it!=end_it;++it)cout<<it->str();
216. 正则表达式中的模式通常包含一个或多个子表达式。一个子表达式是模式的一部分，本身也具有意义。正则表达式使用()表示子表达式。子表达式用于数据验证，验证必须匹配特定格式的数据。
217. 随机数。rand函数返回0-max之间的均匀分布的伪随机整数。random头文件有
        1. 随机数引擎类default_random_engine e;(生成随机unsigned整数序列)e()，返回随机数
        2. 随机数分布类uniform_int_distribution<unsigned> u(0,9);(使用引擎返回服从特定概率分布的随机数)u(e)，返回0-9的随机数

当我们说随机数发生器时，是指分布对象和引擎对象的组合。

218. 随机数发生器的一个特性：即使生成的数看起来是随机的，但对一个给定的发生器，每次程序运行时总返回相同的数值序列。如果需要每次生成不同的数值序列可以把随机发生器定义为static的。如果希望每次运行程序都生成不同结果，可以提供种子来达到这个目的。为引擎设置种子有两种方式：在创建引擎对象时提供种子，或者调用引擎的seed成员。可以直接设置一个整数，也可以调用time函数。
219. 格式化输入与输出。操纵符用来修改流的格式状态。一个操纵符是一个函数或者一个对象，会影响流的状态，并能做完输入或输出运算符的运算对象。操纵符用于两大类输出控制：控制数值的输出格式以及控制补白的数量和位置。当操纵符改变流的格式状态时，通常改变后的状态对所有后续IO都生效。
        1. endl，输出一个换行符并刷新缓冲区
        2. boolalpha，默认bool值打印0/1，boolalpha输出false/true
        3. hex，oct，dec，输出十六进制，八进制，十进制，只影响整型运算对象。
        4. showbase，在输出中指出进制
        5. uppercase，输出大写的十六进制
        6. cout.precision()，浮点数精度，打印的数字的总数，默认为6。setprecision也可以。
        7. scientific，改变流的状态使用科学计数法
        8. fixed，改变流的状态使用定点十进制
        9. showpoint，打印小数点
        10. setw，指定下一个数字或字符串值的最小空间
        11. left/right，左/右对齐输出，默认右对齐
        12. setfill 指定一个字符代替默认的空格来补白输出
        13. noskipws，令输入运算符读取空白符
